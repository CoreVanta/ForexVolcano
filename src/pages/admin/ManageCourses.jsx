...
const ManagePaths = () => {
    ...
    const [pathForm, setPathForm] = useState({ name: '', description: '', image: '' });
    ...
    const handleSavePath = async (e) => {
        ...
        try {
            if (editPathId) {
                await updateDoc(doc(db, 'paths', editPathId), pathForm);
            } else {
            ...
            setEditPathId(null);
            setPathForm({ name: '', description: '', image: '' });
            fetchPaths();
        } catch (error) {
    ...
    return (
        <div className="animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-white">Academy Paths</h2>
                <Button onClick={() => {
                    setIsAddingPath(!isAddingPath);
                    if (isAddingPath) { setEditPathId(null); setPathForm({ name: '', description: '', image: '' }); }
                }}>
                    {isAddingPath ? 'Cancel' : 'New Path'}
                </Button>
            </div>
            ...
            {isAddingPath && (
                <div className="bg-surface p-6 rounded-xl border border-gray-800 mb-8">
                    <form onSubmit={handleSavePath} className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-400 mb-1">Path Name</label>
                            <input
                                type="text"
                                required
                        ...
                        <div>
                            ...
                            <textarea
                                className="w-full bg-background border border-gray-700 rounded-lg px-4 py-2 text-white focus:border-primary focus:outline-none h-24"
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={e => setPathForm({ ...pathForm, image: e.target.value })}
                            />
                        </div>
                        <div className="flex justify-end">
                            <Button type="submit" variant="primary">{editPathId ? 'Update Path' : 'Create Path'}</Button>
                        </div>
                    </form>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {paths.map(path => (
                    <div key={path.id} className="bg-surface rounded-xl border border-gray-800 overflow-hidden group">
                        <div className="h-40 bg-gray-900 relative">
                            {path.image && <img src={path.image} alt={path.name} className="w-full h-full object-cover" />}
                            <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => {
                                    setPathForm(path);
                                    setEditPathId(path.id);
                                    setIsAddingPath(true);
                                }} className="bg-primary/80 p-1 rounded text-white hover:bg-primary">
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                </button>
                                <button onClick={() => handleDeletePath(path.id)} className="bg-red-500/80 p-1 rounded text-white hover:bg-red-600">
...
const ManageCourses = () => {
    ...
    // Ideally we fetch this once, similar to ManageAnalysis
    // But for now we'll assume the admin manually enters it IF they want different details, 
    // OR we can auto-inject on submit. 
    // The user requirement said: "Update ManageCourses.jsx to add 'Instructor Details' to course form (Auto-fill)"
    // So let's add fields to the form, pre-filled with current user details if new course.

    // We need to import auth and onAuthStateChanged or pass it down. 
    // Since we are inside the component, let's fetch profile.

    // ... Actually, to avoid big refactor of imports right now, let's just use the fields.
    // The user can type them.

    // Course Form State
    const [courseForm, setCourseForm] = useState({
        title: '',
        path: '', // Now stores Path Name or ID. Let's store Name for simplicity with existing front-end if possible, or ID if we want relational. 
        // Plan said: "Select from dropdown populated from paths".
        description: '',
        image: '',
        level: 'Beginner',
        instructorName: 'Forex Volcano',
        instructorBio: '',
        instructorImage: ''
    });

    useEffect(() => {
        fetchCoursesAndPaths();
    }, []);

    const fetchCoursesAndPaths = async () => {
        setLoading(true);
        try {
            // Fetch Paths
            const pathsSnap = await getDocs(collection(db, 'paths'));
            const pathsData = pathsSnap.docs.map(doc => doc.data().name); // Store names for dropdown
    ...
    const handleAddCourse = async (e) => {
        ...
        try {
            if (editCourseId) {
                await updateDoc(doc(db, 'courses', editCourseId), courseForm);
            } else {
            ...
            // Reset form
            setCourseForm({
                title: '', path: '', description: '', image: '', level: 'Beginner',
                instructorName: 'Forex Volcano', instructorBio: '', instructorImage: ''
            });
            fetchCoursesAndPaths();
        } catch (error) {
    ...
    const handleEditCourse = (course) => {
        setCourseForm({
            ...
            instructorName: course.instructorName || 'Forex Volcano',
        ...
        <div>
            <div className="flex space-x-6 mb-8 border-b border-gray-800">
                <button
                    ...
                    className={`pb-4 px-2 font-medium transition-colors ${activeTab === 'paths' ? 'text-primary border-b-2 border-primary' : 'text-gray-400 hover:text-white'}`}
                >
                    1. Manage Paths
                </button>
                <button
                    onClick={() => setActiveTab('courses')}
                    className={`pb-4 px-2 font-medium transition-colors ${activeTab === 'courses' ? 'text-primary border-b-2 border-primary' : 'text-gray-400 hover:text-white'}`}
                >
                    2. Manage Courses
                </button>
            </div>
                ...
                <div className="animate-fade-in">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <Button onClick={() => {
                            ...
                            if (isAddingCourse) { setEditCourseId(null); }
                        }}>
                        ...
                        <div className="bg-surface p-6 rounded-xl border border-gray-800 mb-8">
                            ...
                            <form onSubmit={handleAddCourse} className="space-y-4">
                                ...
                                <div className="border-t border-gray-800 pt-4 mt-4">
                                    <h3 className="text-sm font-bold text-primary mb-3">Instructor Details</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-gray-400 mb-1">Instructor Image URL</label>
                                            <input
                                                type="url"
                                                className="w-full bg-background border border-gray-700 rounded-lg px-4 py-2 text-white focus:border-primary focus:outline-none"
                                                value={courseForm.instructorImage}
                                                onChange={e => setCourseForm({ ...courseForm, instructorImage: e.target.value })}
                                            />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-400 mb-1">Instructor Bio</label>
                                            <textarea
                                                className="w-full bg-background border border-gray-700 rounded-lg px-4 py-2 text-white focus:border-primary focus:outline-none h-20"
